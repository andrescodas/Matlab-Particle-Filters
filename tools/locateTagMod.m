...
for j = 1:length(detections)
    if(detections(j).detected)
        ...
        if(tagIndex == 0)
            ...
            tagByParticules = struct('tagId','xxx','particuleSet',particuleSet,'estimatedPosition',[-inf -inf]);
...
for tagIndex = 1:length(inferingTags);
    inferingTags(tagIndex).estimatedPosition = estimateTagPosition(inferingTags(tagIndex).particuleSet);
end
